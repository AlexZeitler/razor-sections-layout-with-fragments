...
You can use these commands, e.g. `RazorSectionsLayouts`  gets renamed to `MyApp`:
...
```shell
LC_ALL=C find . -type f -name '*.*' -not \( -path './node_modules/*' -o -path './src/RazorSectionsLayouts/node_modules/*' -o -path './assets' \) -exec sed -i '' 's|RazorSectionsLayouts|MyApp|g' {} \;
```
...
```bash
find . -depth -name "*RazorSectionsLayouts*" | \
while IFS= read -r ent; do mv $ent ${ent%RazorSectionsLayouts*}MyApp${ent##*RazorSectionsLayouts}; done
